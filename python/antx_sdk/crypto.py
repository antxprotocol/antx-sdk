...
def convert_to_antx_addr(addr: str, account_hrp: str = ACCOUNT_HRP) -> str:
    if not addr:
        raise ValueError("addr can't be empty")
    if addr.startswith("0x") and len(addr) == 42:
...
def derive_antx_bech32_address(private_key_bytes: bytes, account_hrp: str = ACCOUNT_HRP) -> str:
